...
data LogLine = MkLine {
  logLineDatetime :: LocalTime,
  logLineAction :: GuardAction
} deriving (Show)

data GuardAction = BeginShift Int |
                   FallAsleep |
                   WakeUp deriving (Show, Eq)

guardAction :: Parser GuardAction
guardAction = string "Guard #" *> (BeginShift <$> guardIdShiftBegin)
          <|> string "falls asleep" *> pure FallAsleep 
          <|> string "wakes up" *> pure WakeUp

guardIdShiftBegin :: Parser Int
guardIdShiftBegin = do guardId <- many1 digit
                       string " begins shift"
                       return (read guardId :: Int)
...
logLine = do char '['
             ...
             return MkLine {
                 ...
                 logLineAction = action
             }

logList :: Parser [LogLine]
logList = do l <- many logLine
             eof
             return l

parseLog i = parse logList "" i

-- Answers

advent4_1 = do logs <- parseLog input
               let sortedLogs = sortBy (comparing logLineDatetime) logs
               return sortedLogs
...
input2 = [here|[1518-11-01 00:00] Guard #10 begins shift
...
input = [here|[1518-09-20 00:43] falls asleep
